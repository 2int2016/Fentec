...
class LogarController{  
	...
	public function autenticar($usu){
		$usu = $this->object_to_array($usu);
		$model = LogarModel::find('all', array('conditions' => array('email = ? AND senha = ?', $usu['email'], $usu['senha'])));
		
		if(isset($model[0])){
			$_SESSION['id_user'] = $model[0]->id;
	...
	public function buscar(){
		if(isset($_SESSION['id_user'])){
			$model = VisitanteModel::find($_SESSION['id_user']);
			return $model->to_array();
		}
		return 0;
	}
